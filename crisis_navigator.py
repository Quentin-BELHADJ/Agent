...
def load_system_prompt() -> str:
    ...
    """
    path = AGENTS_DIR / ".gemini" / "agents" / "crisis-navigator.md"
    if not path.exists():
        path = AGENTS_DIR / "crisis-navigator.md"
...
def main() -> int:
    import argparse
    parser = argparse.ArgumentParser(description="Crisis Navigator — Agent d'Évacuation d'Urgence")
    parser.add_argument("query", type=str, nargs="?", help="Requête de l'utilisateur")
    args = parser.parse_args()
    
    if not args.query:
        print("Erreur : Veuillez spécifier une requête.", file=sys.stderr)
        print("Usage : python3 crisis_navigator.py \"<requête>\"", file=sys.stderr)
        return 1
